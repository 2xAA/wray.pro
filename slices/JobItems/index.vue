...
<template>
  ...
  <r-cell span="8">
    <r-grid columns="8">
      <r-cell span="8" class="job" v-for="{job} in slices" :key="job.id">
        <h2>{{job.data.title[0].text}}</h2>

        <r-grid columns="8">
          <r-cell span="1.." v-for=" { role, description, date, end_date, present_job }, index in job.data.roles" :key="`${job.id}-${index}`">
            <r-grid columns="8">
          ...
          <r-cell span="8" v-if="job.data.related_work?.length">
            <h4>Related Work</h4>
            <r-grid columns="6" class="related_work">
              <r-cell span="1" span-s="2" v-for="{ work } in job.data.related_work" :key="work.id">
